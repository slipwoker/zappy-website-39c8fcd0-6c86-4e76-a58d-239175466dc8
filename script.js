document.addEventListener('DOMContentLoaded', function() {
  ...
  contactForms.forEach(function(form) {
    ...
    form.addEventListener('submit', function(e) {
      ...
      var previousErrors = form.querySelectorAll('.field-error');
      previousErrors.forEach(function(el) {
        el.remove();
      });
      var previousInvalids = form.querySelectorAll('.invalid');
      previousInvalids.forEach(function(el) {
  ...
  function validateField(field) {
    ...
    if (type === 'hidden' || type === 'submit' || type === 'button' || tagName === 'button') {
      return null;
    }
    ...
    if (pattern) {
      ...
      if (!patternRegex.test(value)) {
        ...
        return title || 'Please match the required format.';
      }
    }
  ...
  var animatedElements = document.querySelectorAll('.fade-in, .fade-in-up, .fade-in-left, .fade-in-right, [data-animate]');
  ...
  if (animatedElements.length > 0) {
    ...
    style.textContent =
      ...
      '.fade-in-right {' +
        'transform: translateX(30px);' +
      '}' +
      '.fade-in.is-visible, .fade-in-up.is-visible, .fade-in-left.is-visible, .fade-in-right.is-visible, [data-animate].is-visible {' +
        'opacity: 1;' +
        'transform: translate(0, 0);' +
      '}';
    document.head.appendChild(style);

    // RTL: swap left/right animations
    var isRTL = document.documentElement.getAttribute('dir') === 'rtl' ||
                getComputedStyle(document.documentElement).direction === 'rtl';
    if (isRTL) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "עידו רכבים משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
        ...
        function initContactForm() {
            ...
            try {
                ...
                const response = await fetch(endpoint, {
                    ...
                    body: JSON.stringify({
                        websiteId: '39c8fcd0-6c86-4e76-a58d-239175466dc8',
                        name: data.name,
                        email: data.email,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '39c8fcd0-6c86-4e76-a58d-239175466dc8';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '39c8fcd0-6c86-4e76-a58d-239175466dc8';
